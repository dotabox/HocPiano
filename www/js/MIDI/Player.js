...
var scheduleTracking = function (channel, note, currentTime, offset, message, velocity, track) {
...
var startAudio = function (currentTime, fromCache) {
	...
	for (var n = 0; n < length && messages < 100; n++) {
		...
		if(n<=maxN && fromCache) continue;
		currentTime = queuedTime - offset;
		var event = data[n][0].event;
		var track = data[n][0].track;
		...
		switch (event.subtype) {
			case 'noteOn':
				...
				eventQueue.push({
					event: event,
					source: MIDI.noteOn(channel, event.noteNumber, 0, currentTime / 1000 + ctx.currentTime),
					interval: scheduleTracking(channel, note, queuedTime, offset, 144, event.velocity,track)
				});
				messages ++;
				break;
			case 'noteOff':
